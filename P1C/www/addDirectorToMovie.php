    ...
    <div class="container">
      <div class="top-nav">
          ...
          <h6>
            ...
            <li><a href="addDirectorToMovie.php" class="active">Add a Movie/Director Relation</a></li>
          </h6>
          </ul>
          <!-- script-for-menu -->
            <script>
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
   ...
   $director = test_input($_GET["actor"]);
}
...
<form method="get" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> 
 ...
 <?php
   $sql = "SELECT first,last,dob,id FROM Director ORDER BY first ASC;";
   $rs = $conn->query($sql);
   $row = $rs->fetch_assoc();
    while($row = $rs->fetch_assoc()) {
    $fname = $row["first"];
...
if ($conn->query($sql) === TRUE) {
    ...
    //$conn->query($sql);
} elseif($conn->query($sql) === FALSE && $director!= "" && $movie!="") {
